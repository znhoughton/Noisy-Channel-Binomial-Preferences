...
\urlstyle{same}
\hypersetup{
  pdftitle={The Bolts and Nuts of Language Processing:},
...
\citeproc{ref-chomskyAspectsTheorySyntax1965}{Chomsky, 1965};
\citeproc{ref-pinkerFutureTense2002}{Pinker \& Ullman, 2002}). For
...
\citeproc{ref-culicover2017multiwordconstructionsgrammar}{Culicover et
al., 2017}). For example, Culicover et al.
(\citeproc{ref-culicover2017multiwordconstructionsgrammar}{2017})
...
Similarly, there is evidence that phonetic representations may be stored
holistically (i.e., as exemplars instead of abstract phoneme
representations). For example, in their seminal paper McMurray et al.
...
representations of sounds (i.e., phonemes). While it may be possible to
account for the results in McMurray et al.
(\citeproc{ref-mcmurrayGradientSensitivityWithincategory2008}{2008})
without storing exemplars (e.g., by using the phonetic detail to
facilitate the judgement of whether a sound is /p/ or /b/ but
none-the-less still implementing a categorical judgement), the phonetic
information must be stored somewhere (because participants must have
some knowledge of the distribution of VOTs for a /p/ or a /b/). As such,
this finding has been taken as evidence that people are storing
exemplars of sounds when they experience them as opposed to only storing
abstract representations. While this is a phenomenon at the individual
...
The evidence for item-specific representations of sounds naturally
sparked similar theoretical debates about the representations of
multi-word phrases. For example, Bybee \& Scheibman
...
individual parts.\footnote{One could account for this by positing
  ...
  Scheibman (\citeproc{ref-bybeeEffectUsageDegrees1999}{1999}) since
  even in the phrase \emph{I don't know, don't} seems to only be
  reducible in specific contexts. For example, in sentences that contain
  an additional clause (e.g., in a sentence containing a complementizer
...
2023}; \citeproc{ref-morganModelingIdiosyncraticPreferences2015}{Morgan
...
(\citeproc{ref-siyanova-chanturiaSeeingPhraseTime2011}{2011}) since the
...
\emph{dogs and cats}).\footnote{Although see Frazier et al.
  (\citeproc{ref-frazier2000processingcoordinatestructures}{2000}) for
  ...
  et al. (\citeproc{ref-siyanova-chanturiaSeeingPhraseTime2011}{2011}).}
...
examined whether human ordering preferences are driven by these abstract
preferences for binomials ranging from low to high overall frequency
...
Specifically, they used a self-paced reading task where participants
read binomials in sentence contexts. They found that for low-frequency
binomials, human reading times were driven primarily by generative
preferences, however for high-frequency binomials human reading times
were driven primarily by relative frequency (i.e., the proportion of
counts in the alphabetical ordering to the nonalphabetical ordering).
Their results suggest that humans compose low-frequency binomials using
abstract knowledge of the language, however for high-frequency binomials
humans store and access a holistic representation of the entire
binomial.
...
2016a}). A great deal of evidence for this phenomenon comes from the
...
Levy, 2015}; e.g., a strong preference for \emph{bread and butter} over
...
One way to account for frequency-dependent preference extremity is to
posit that high-frequency binomials are stored holistically. For
example, as mentioned earlier there is evidence that humans rely on
abstract preferences for low-frequency binomials but rely on their
experience with a binomial for high-frequency binomials. This might help
to explain why people develop strong preferences for high-frequency
binomials but not low-frequency binomials. However, while holistic
storage may be a driving factor for frequency-dependent preference
extremity, it isn't enough by itself to account for this phenomenon.
That is, high-frequency binomials being stored holistically does not
necessarily lead to them being used more.
...
overcome their prior knowledge and produce more regularized/polarized
productions.
...
The learner estimates this by multiplying the probability of observing
the alphabetical ordering of a given binomial under a given hypothesis
about the ordering preferences (the likelihood, \(P(x_1|\theta_1)\),
Equation~\ref{eq-likelihoodbernoulli}) by the prior probability of that
hypothesis (\(P(\theta_1)\), Equation~\ref{eq-priorbeta}).
...
prior\footnote{Note that \(\alpha\) is specified a-priori and determines
  ...
  (\citeproc{ref-realiEvolutionFrequencyDistributions2009}{2009}).}
...
the binomial). For low-frequency binomials, the strength of the
regularization is not strong enough to overcome the prior, and thus
learners don't develop strong preferences for low-frequency binomials.
On the other hand, high-frequency binomials occur enough that the
regularization bias can overcome the prior and thus learners learn more
polarized preferences (sometimes even in the opposite direction from the
abstract preferences). In other words, this model was able to predict
...
increases (i.e., as the regularization parameter increases), the
strength of the regularization effect increases (as demonstrated by
greater probability density around values of 0 and 1). While their
results demonstrate that a frequency-independent regularization bias can
account for frequency-dependent preference extremity, it's unclear what
process this bias is analogous to in language learning/processing.
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{Figures/morgan_freq_dep_reg_results.pdf}
...
(\citeproc{ref-feltyMisperceptionsSpokenWords}{2013}) found that when
...
also increases (i.e., an increase in the value of
...
nonalphabetical order while updating their hypothesis about the
...
preferences, which may result in the learner developing more polarized
preferences for that binomial. However, for low-frequency binomials, the
learner does not encounter the binomial as many times and has fewer
opportunities to update their beliefs, which may result in less
...
Levy (\citeproc{ref-morganModelingIdiosyncraticPreferences2015}{2015}).}

\centering{

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{noisy-channel-binomial-preferences-writeup_files/figure-pdf/fig-corpusourmodel-1.pdf}
...
2011}). Additionally, all of our items had a relative frequency of
...
\toprule
\textbf{Condition} & \textbf{Target Freq.} & \textbf{Prime Freq.} & \textbf{Target Binomial} & \textbf{Prime Binomial}\\
\midrule
...
\citeproc{ref-morganAbstractKnowledgeDirect2016}{Morgan \& Levy,
2016a}). We also excluded participants who had an average accuracy below
...
\citeproc{ref-morganAbstractKnowledgeDirect2016}{Morgan \& Levy,
2016a}). We then summed the residual reading times for each word in the
...
storage (\citeproc{ref-arnonMoreWordsFrequency2010}{Arnon \& Snider,
2010}; \citeproc{ref-bybeePhonologyLanguageUse2003}{Bybee, 2003};
...
Radicke, 2009};
\citeproc{ref-morganFormalizingConstructionGrammar}{Morgan et al.,
2023}; \citeproc{ref-morganAbstractKnowledgeDirect2016}{Morgan \& Levy,
2016a}, \citeproc{ref-morganProductiveKnowledgeItemspecific2024}{2024}).
...
(\citeproc{ref-kapatsinskiFrequencyEmergencePrefabs2009}{Kapatsinski \&
Radicke, 2009}). It is also the case that holistically stored
multi-morphemic words are less susceptible to speech errors
(\citeproc{ref-stembergerAreInflectedForms2004}{Stemberger \&
MacWhinney, 2004}). Given these results, a holistically stored binomial
...
better learning due to increased attention to the surprising item
(\citeproc{ref-haeuser2023effects}{Haeuser \& Kray, 2023};
\citeproc{ref-hollingworth2000semantic}{Hollingworth \& Henderson,
2000}; \citeproc{ref-hubbard2024impact}{Hubbard \& Federmeier, 2024};
\citeproc{ref-olejarczuk2018}{Olejarczuk et al., 2018}). For example,
Haeuser \& Kray (\citeproc{ref-haeuser2023effects}{2023}) used a
...
attract attention and tend to be remembered better
(\citeproc{ref-hollingworth2000semantic}{Hollingworth \& Henderson,
2000}). For example, Hollingworth \& Henderson
(\citeproc{ref-hollingworth2000semantic}{2000}) found that participants
...
the present study suggest that it is possible that the surprising nature
of the infrequent ordering of the binomials draws enough attention to
them to negate any noisy-channel processing effects.
...
Levy, 2016}). Further, word and phrase exchanges are a very common type
of speech error (\citeproc{ref-fromkin1973speech}{Fromkin \& Fromkin,
1973}), so a robust system of language processing should be able to
...
Levy (\citeproc{ref-poppelsStructuresensitiveNoiseInference2016}{2016})
...
accessible (\citeproc{ref-harmonPuttingOldTools2017}{Harmon \&
Kapatsinski, 2017}). For high-frequency binomials, if one ordering is
...
\bibitem[\citeproctext]{ref-bybeePhonologyLanguageUse2003}
...
\bibitem[\citeproctext]{ref-culicover2017multiwordconstructionsgrammar}
...
\bibitem[\citeproctext]{ref-frazier2000processingcoordinatestructures}
...
\bibitem[\citeproctext]{ref-fromkin1973speech}
...
\bibitem[\citeproctext]{ref-haeuser2023effects}
...
\bibitem[\citeproctext]{ref-hollingworth2000semantic}
...
\bibitem[\citeproctext]{ref-hubbard2024impact}
